...
Let $C$ and $D$ be cycles in $G$. Which of the following scenarios are _not_ feasible?
...
if X is not NO:
    return X U {v}
otherwise:
    return solve-CCHS(G-v,F U {v},k) // omit v
```


**Option B.**

<!-- Issue here is k-1 is superflous -->
...
if X is not NO:
    return X U {v}
otherwise:
    return solve-CCHS(G-v,F U {v},k-1) // omit v
```


**Option C.**

<!-- Issue here is X U {v} is missing -->
...
<!-- Issue here is F U {v} is missing -->
...
if X is not NO:
    return X U {v}
otherwise:
    return solve-CCHS(G-v,F,k) // omit v
```
...
In the next few questions, we will analyze the running time of this algorithm. Assume that the algorithm is called with $F = \emptyset$, i.e, there are no forbidden vertices to begin with. For a fixed execution path $P$ that does _not_ return NO, we use $S(P)$ to denote the output: note that this would consist of the set of vertices that are removed from the instance for inclusion in the solution, i.e, vertices chosen by the first branch in Step 1.6. Also use $F(P)$ to denote the set of vertices that are made forbidden along the path $P$, i.e, these are vertices chosen by the second branch in Step 1.6. 

Note that at the output node, the instance we have is $(G,F(P),\ell)$ for some $\ell \leq k$, where $S(P)$ is a constrained cycle hitting set with respect to $F(P)$ of size at most $k - \ell$.
...
Fix an execution path $P$ on an instance $(G,\emptyset,k)$ and consider a vertex $v$ that belongs to $F(P)$, which is to say that the vertex $v$ was included in the set of forbidden vertices at some point during the execution of the algorithm. Denote the instance at this stage of the algorithm by $(H,F,\ell)$ Let $d(v)$ denote the degree of the vertex $v$ in the original instance $G$, and let $d^\star(v)$ denote its degree in the graph $H$. Which of the following is true?
...
Describe a 0/1-LP that models the problem correctly. Also describe a $5$-approximation assuming that each point is contained in at most 5 rectangles. 
:::
...
Let $S$ be a dominating set of $G$ of size at most $k$, where $G$ is as given above (i.e, $G$ has no triangles and no cycles of length four). Show that if the degree of $v$ is more than $k$ in $G$, then $v \in G$.
:::
