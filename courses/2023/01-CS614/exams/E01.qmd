...
Fix an execution path $P$ on an instance $(G,\emptyset,k)$ and consider a vertex $v$ that belongs to $F(P)$, which is to say that the vertex $v$ was included in the set of forbidden vertices at some point during the execution of the algorithm. Denote the instance at this stage of the algorithm by $(H,F,k^\prime)$ Let $d(v)$ denote the degree of the vertex $v$ in the original instance $G$, and let $d^\star(v)$ denote its degree in the graph $H$. Which of the following is true?
