...
3. Player 2 wins because notice that no matter how the first player plays the first $(n-1)$ steps, it is not enough to disconnect the graph. By chosing edges carefully^[e.g, by choosing an edge incident on the $i$-th vertex in the $i$-th move that connectes the $i$-th vertex to the subgraph built so far by the second player], Player 2 can ensure that (s)he has booked a spanning tree already within the first $(n-1)$ moves.
