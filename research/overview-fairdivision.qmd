...
There are several notions of "happiness" proposed in the fair division literature. Suppose we are trying to distribute $R$^[I will implicitly assume the indivisble setting, but the ideas carry over naturally to cake cutting.] among a set of $n$ agents $A$ whose valuations are given by $v$, and we have come up with a distribution $D$. For an agent $a$, we use $D_a$ to denote the part labeled $a$ in $D$. 
...
If any of this piqued your interest, you can find below a list of pointers to find out more.
